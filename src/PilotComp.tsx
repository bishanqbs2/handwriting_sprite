...
function PilotComp() {
  ...
  const [pages, setPages] = useState(["Home", "Before", "Lowercase", "Uppercase", "Num", "Words", "Own",] as any);
  ...
  const preload = () => {
    const sItems = AsyncPreloader.loadItems(items);
    sItems
      .then((items) => {
        console.log(items);
      })
  ...
  return (
    <GlobalContext.Provider
      value={{
        state, dispatch, guidlinesToggle, setGuidlinesToggle, traceToggle, setTraceToggle,
        lowerCaseLetters, upperCaseLetters, numbers, data, selectedNode, setSelectedNode,
        activeLetter, setActiveLetter, selectedWord, setSelectedWord, filteredWords,
        setFilteredWords, wordindex, setWordindex, newWord, setNewWord, onClickState,
        SetonClickState, traced, setTraced, playonetime, setPlayone
      }}
    >
      <HashRouter>
